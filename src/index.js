...
//[PK] -- code here looks good, but could definitely benefit from more modularity, specifically breaking things out into separate files
...
class App extends Component {
	...
	async componentDidMount() {
		//[PK] -- sooo terse... maybe too terse!
		//[PK] -- (my auto-formatter added in the line breaks here, which makes this actually look pretty good. so i maybe take it back. but you really need the line breaks!)
		this.setState({
			...
			<div>
				...
				<div id="all-depts">
					<div id="no-dept" className="dept">
						...
						{employees.filter(e => e.departmentId === null).length})
						{displayNoDept()}
					</div>
					{displayDepts()}
				</div>
					...
					.map(employee => {
						return (
							<li key={employee.id}>
								...
								<button onClick={() => changeDept(employee, "add")}>
		...
		return departments.map(department => {
			return (
				<div key={department.id} className="dept">
					{department.name} (
					{employees.filter(e => e.departmentId === department.id).length})
					<ul id="employees">
						{employees
							...
							.map(employee => {
								return (
									<li key={employee.id}>
										<p>{employee.name}</p>
										<button onClick={() => destroy(employee)}>x</button>
										<button onClick={() => changeDept(employee, "remove")}>
